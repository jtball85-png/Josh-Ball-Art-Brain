...
Ask via whatever structured-question tool this Claude surface provides (e.g. AskUserQuestion in Claude Code), but keep it conversational. If context is already available from the conversation (e.g. the user already shared the URL and described their product), skip those questions and confirm your understanding instead.
...
If a browser/screenshot tool genuinely is available in this session, use it and report real visual findings instead of inferring from text. Otherwise, always label anything in this layer as inferred from structure/text, never as a directly observed visual fact.
...
Save the audit as a markdown file in this project's own research/output location (in this repo: `garage/research/`), not a path from another Claude surface. Name it `[site-name]-website-audit-[date].md`.
...
*Installed from https://github.com/appariciojunior/website-audit-skill (original by ajota.uk) with two adaptations for this repo/environment: the discovery-tool reference and the output path above. See `checklist.md` in this folder for the detailed per-item checklist.*
